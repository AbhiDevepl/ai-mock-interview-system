...
## 2026-07-28 - Lean Queries and Direct Atomic updateOne for High-Performance Document Finish Actions
**Learning:** Finishing an interview requires reading calculated nested array fields to construct average scores, then saving the overall score and changing status to 'complete'. Relying on a standard `.save()` operation on a hydrated model after reading the document runs full Mongoose tracking validation over the entire `questions` array schema. Instead, using `.select("userId questions").lean()` fetches a tiny lightweight object, and then calling `updateOne` directly writes the results atomically, avoiding massive CPU/memory overhead.
**Action:** When finalising document summaries or status fields computed from arrays, read via a selective `.lean()` query and persist results with direct `updateOne` statements.
