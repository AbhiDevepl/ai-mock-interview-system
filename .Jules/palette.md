...
## 2025-03-04 - High-Contrast File Dismissal & Native Key Binding Reset
**Learning:** File upload features often lock users into their selection once a file is uploaded, with no easy way to discard or start over. Providing high-contrast, descriptive, keyboard-navigable 'remove' buttons adjacent to the file trigger and inside the modal is essential for full accessibility. Furthermore, React state updates to clear the file will not reset the native file input element's browser cache unless we dynamically bind the `key` attribute of the input to the selected file's name.
**Action:** Always place accessible remove buttons next to file triggers and inside upload modals, and dynamically reset native file inputs using the file name or fallback key.