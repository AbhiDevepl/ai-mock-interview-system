...
## 2025-03-06 - Dynamic File Input Keys & Inline Discard Elements
**Learning:** When developing stateful React file upload inputs, resetting parent state properties does not automatically clear native input buffers, leading to UX friction where selecting the exact same file after discarding fails to fire standard `onChange` hooks. Coupling dynamic element key binding (`key={file ? file.name : "empty"}`) with dedicated inline "Remove" action controls provides robust state serialization and a delightful, predictable interactive feedback loop.
**Action:** Always bind the `key` attribute of an `input[type="file"]` to the selected file's metadata to ensure absolute DOM synchronization, and provide explicit dual remove triggers (both within the parent view and modal dialogues).
