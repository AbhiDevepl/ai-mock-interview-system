...
## 2025-03-04 - React File Input State Reset using Dynamic Key Binding
**Learning:** Standard React hidden file inputs mapped to custom visual upload trigger buttons can become out of sync or fail to trigger the `onChange` event if a user removes a selected file and immediately attempts to re-select/upload the exact same file. This is because the browser's native `<input type="file">` still retains the previous value, and does not fire a change event. Dynamically binding the input's `key` attribute to the selected file's name or a default fallback forces a clean component re-render on file removal, solving browser state out-of-sync issues gracefully.
**Action:** Always key native file inputs dynamically based on the state of the selected file (`key={file ? file.name : 'empty'}`) when implementing custom styled triggers to guarantee that removal actions successfully reset the native DOM element's value.