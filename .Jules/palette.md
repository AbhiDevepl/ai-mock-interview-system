...
## 2025-03-01 - Keyboard Focusable Branding Buttons in Headers
**Learning:** Logos and branding labels in headers are often made clickable via `cursor-pointer` but remain entirely unreachable to keyboard users because they are styled static `div` containers. Converting these wrappers into semantic HTML `<button>` elements with keyboard focus outline rings (`focus-visible:ring-2`), hover backdrops, and descriptive screen-reader tags (`aria-label`) restores native accessibility expectations seamlessly.
**Action:** When designing headers or navigation bars, ensure branding logos with `cursor-pointer` are converted into semantic, keyboard-focusable `<button>` elements with clear navigation handlers, focus outline rings (`focus-visible:ring-2`), hover backdrops, and descriptive screen-reader labels (`aria-label`).
