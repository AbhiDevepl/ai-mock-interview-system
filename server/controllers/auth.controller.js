...
export const googleAuth = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      // Prioritize verified metadata from Firebase token over client data
      if (verifiedName) user.name = verifiedName;
      if (verifiedPicture) user.picture = verifiedPicture;

      // Fallback to client data only if not present in token
      if (!verifiedPicture && photo) user.picture = photo;
