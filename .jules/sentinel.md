...
## 2026-07-18 - [HIGH] Input Validation Gap, Schema Mismatch, and Prompt Injection in Interview Endpoints
**Vulnerability:** The interview controller lacked type-checking, bounds/length limits, and schema enum mapping for incoming body parameters. This allowed prompt injection via un-validated inputs, unhandled `CastError` / `TypeError` exceptions returning generic 500 server errors, and client credit-deduction loss due to database ValidationErrors triggered by unmatched mode enums (`Behavioral` vs `HR`).
**Learning:** Weak parameter sanitization, lack of type validation, and direct interpolation of untrusted input strings into LLM system prompts can lead to injection attacks and unhandled exceptions. Furthermore, mismatch between client-facing strings and DB enums can cause late-stage transaction failures.
**Prevention:** Enforce strict type constraints, string length caps, and enum validation at the controller boundary. Map client values to database-compatible enums before executing state-changing logic or external API calls, and gracefully reject invalid input formats with standard 400 Bad Request responses.
